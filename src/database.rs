...
const DATABASE_CREATE_STMT: &str = concat!("CREATE TABLE IF NOT EXISTS Data ",
...
const DATABASE_INSERT: &str = concat!("INSERT INTO Data(", "battery_voltage, ", "pv_voltage, ",
"load_current, ", "over_discharge,", "battery_max, ", "battery_full, ", "charging, ",
"battery_temp, ", "charge_current, ","load_onoff,", "time", ") VALUES(?,?,?,?,?,?,?,?,?,?,?)");
...
impl Database {
    ...
    pub(crate) fn add_datapoint(&mut self, datapoint: DataPoint) {
        ...
        if self.datapoint_buffer.len() >= BUFFER_LIMIT {
            let data = mem::replace(&mut self.datapoint_buffer,
    ...
    fn insert_datapoints(&mut self, datapoints: Vec<DataPoint>) {
        ...
        for dp in datapoints {
            match trans.execute(DATABASE_INSERT, 
